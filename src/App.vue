<template>
  <div class="relative w-96 bg-slate-50 text-slate-800 font-sans shadow-2xl flex flex-col h-[540px] overflow-hidden">
    ...
    <div v-if="currentView === 'home'" class="p-4 flex-1 flex flex-col space-y-4 overflow-y-auto custom-scrollbar relative z-10">
      
      <div class="grid grid-cols-2 gap-3 shrink-0">
        <div class="bg-white border border-slate-200 p-3 rounded-xl flex flex-col items-start justify-center shadow-sm relative overflow-hidden">
          <div v-if="isRunning" class="absolute inset-0 bg-emerald-50 opacity-50 animate-pulse"></div>
          <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1 relative z-10">Sistem Durumu</span>
          <span :class="isRunning ? 'text-emerald-600' : 'text-slate-600'" class="text-sm font-extrabold flex items-center gap-1.5 relative z-10">
            <span v-if="isRunning" class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
            <span v-else class="w-2 h-2 rounded-full bg-slate-300"></span>
            {{ isRunning ? 'Aktif Döngü' : 'Beklemede' }}
          </span>
        </div>
        
        <div class="bg-indigo-50/50 border border-indigo-100 p-3 rounded-xl flex flex-col relative shadow-sm">
          <div class="flex items-center justify-between w-full mb-0.5">
            <span class="text-[10px] text-indigo-500 font-bold uppercase tracking-wider">Gerçekleşen İşlem</span>
            <button @click="resetCounter" :disabled="totalActionsCount === 0" class="text-indigo-400 hover:text-indigo-600 disabled:opacity-30 transition-colors" title="Sayacı Sıfırla">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 flex-1 flex flex-col">
        ...
        <div class="p-4 border-b border-slate-100 bg-slate-50/50 shrink-0">
          <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <span class="bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded text-[9px]">1</span> DOM Hedefi Belirle
          </label>
          <div class="flex gap-2">
            <input v-model="targetSelector" type="text" readonly class="flex-1 border border-slate-200 p-2 rounded-md text-xs bg-white text-slate-600 focus:outline-none focus:border-indigo-300 shadow-sm" placeholder="Tıklama/Yazma hedefi seçin..." />
            <button @click="startInspection" :disabled="isRunning" class="bg-[#020617] text-white px-4 py-2 rounded-md text-xs font-semibold hover:bg-slate-800 transition-all disabled:opacity-50 shadow-sm">Seç</button>
          </div>
        </div>
        ...
        <div class="p-4 flex-1 flex flex-col justify-center">
          <div class="flex items-center justify-between mb-3">
            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
              <span class="bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded text-[9px]">2</span> Aktif Senaryo
            </label>
            <button v-if="isRealPackageSelected" @click="openForm(selectedPackageId)" :disabled="isRunning" class="text-[10px] font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1 disabled:opacity-50 transition-colors bg-indigo-50 px-2 py-1 rounded-md">
              <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
              Düzenle
            </button>
          </div>

          <div v-if="isRealPackageSelected" class="bg-gradient-to-br from-white to-slate-50 border border-slate-200 rounded-xl p-3.5 shadow-sm flex items-center justify-between group transition-all hover:border-indigo-200 hover:shadow-md">
            <div class="flex items-center gap-3 overflow-hidden">
              <div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center shrink-0">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
              </div>
              <div class="flex flex-col truncate">
                <span class="text-sm font-bold text-[#020617] truncate">{{ selectedPackageName }}</span>
                <span class="text-[10px] font-medium text-slate-500 mt-0.5">
                  {{ selectedPackageData?.actions?.length || 0 }} Aksiyon Döngüsü
            ...
            <button @click="showScenarioModal = true" :disabled="isRunning" class="ml-2 bg-white border border-slate-200 text-slate-600 hover:text-indigo-600 hover:border-indigo-300 text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-sm transition-all disabled:opacity-50 shrink-0">
              Değiştir
            </button>
          </div>

          <div v-else class="bg-slate-50 rounded-xl border border-dashed border-slate-300 text-center flex-1 flex flex-col items-center justify-center p-4">
            <div class="w-10 h-10 bg-white rounded-full shadow-sm flex items-center justify-center border border-slate-100 mb-2">
              <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
            </div>
            <p class="text-[11px] text-slate-500 mb-3 font-medium">İşlem yapabilmek için bir senaryo atayın.</p>
            <button @click="showScenarioModal = true" class="text-xs bg-white border border-slate-200 text-[#020617] px-4 py-2 rounded-lg font-bold hover:bg-slate-100 transition-all shadow-sm">
              Senaryo Seç veya Yarat
            </button>
          </div>
          
          <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-center gap-1.5 opacity-80">
            <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
            <span class="text-[10px] font-medium text-slate-400">Sistemde toplam <strong class="text-slate-500">{{ macroPackages.length }}</strong> senaryo kayıtlı</span>
          </div>
    ...
    <div v-else-if="currentView === 'form'" class="p-4 flex-1 flex flex-col bg-white overflow-hidden">
      ...
      <div class="flex items-center justify-between mb-3 shrink-0">
        ...
        <button v-if="formMode === 'edit'" @click="showDeleteModal = true" class="text-xs text-rose-500 hover:text-rose-700 font-semibold flex items-center gap-1 bg-rose-50 px-2.5 py-1 rounded transition-colors">
      ...
      <div class="flex-1 overflow-y-auto pr-2 space-y-3 mb-4 custom-scrollbar">
        ...
        <div v-for="(action, index) in packageForm.actions" :key="index" 
          ...
          <div class="flex items-center justify-between">
            <span class="bg-slate-100 text-slate-500 text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider select-none">
              #{{ index + 1 }}
            </span>
            ...
            <div class="flex items-center gap-2">
              ...
              <button 
                @click="action.isActive = !action.isActive" 
                :class="action.isActive ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'" 
                class="flex items-center gap-1 px-2 py-0.5 rounded-md border text-[10px] font-bold transition-all shadow-sm" 
                title="Adımı Aktif veya Pasif Yap">
                <span :class="action.isActive ? 'bg-emerald-500' : 'bg-slate-400'" class="w-1.5 h-1.5 rounded-full transition-colors"></span>
                {{ action.isActive ? 'Aktif' : 'Pasif' }}
              </button>

              <div class="flex items-center bg-slate-100 rounded-md p-0.5 text-[10px] font-bold text-slate-500 border border-slate-200/50 shadow-inner">
          ...
          <div class="flex items-stretch gap-2">
            <button @click="action.isClick = !action.isClick" :class="action.isClick ? 'bg-[#020617] text-white border-[#020617] shadow-inner' : 'bg-slate-50 text-slate-400 border-slate-200 hover:bg-white hover:text-slate-600'" class="w-8 shrink-0 rounded-md border transition-all flex items-center justify-center" title="Tıklama event'i">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5"></path></svg>
            </button>
    ...
    <div v-if="showScenarioModal" class="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-start justify-center pt-16 px-4 animate-in fade-in duration-150">
      <div class="bg-white w-full rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[400px] border border-slate-200/50 transform transition-all">
        ...
        <div class="p-3 border-b border-slate-100 flex items-center gap-3 bg-slate-50/50">
          ...
          <input v-model="searchQuery" type="text" class="flex-1 bg-transparent border-none text-sm focus:ring-0 focus:outline-none font-medium text-slate-700 placeholder-slate-400" placeholder="Senaryo adı ara..." ref="searchInputRef" autofocus />
          <button @click="showScenarioModal = false" class="p-1.5 text-slate-400 hover:text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                  <span class="text-xs font-bold text-slate-800" :class="selectedPackageId === pkg.id ? 'text-indigo-900' : ''">{{ pkg.name }}</span>
                  <span class="text-[10px] text-slate-500">{{ pkg.actions?.length || 0 }} Aksiyon</span>
                </div>
              </div>
              <svg v-if="selectedPackageId === pkg.id" class="w-4 h-4 text-indigo-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
            </button>
          </div>
        </div>

        <div class="p-2 border-t border-slate-100 bg-slate-50/80">
...
const storageListener = (changes, namespace) => {
  if (namespace === 'local' && changes.totalActionsCount) {
    totalActionsCount.value = changes.totalActionsCount.newValue || 0;
  }
}

onMounted(() => {
  if (chrome?.storage) {
    chrome.storage.local.get(['savedPackages', 'savedSelector', 'selectedPackageId', 'isRunning', 'totalActionsCount'], (result) => {
...
const toggleMacro = async () => {
  ...
  if (!isRunning.value) {
    ...
    chrome.tabs.sendMessage(tab.id, { action: 'START_PACKAGE_MACRO', config: { selector: targetSelector.value, actions: actionsForMacro } })
  } else {
